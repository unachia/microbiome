...
selected = pd.read_csv(
    "data/cardiometabolic_marker_postprandial.tsv", sep="\t", index_col=0
)
...
# read df
df = pd.read_csv(
    "results/diversity/metaphlan_riskgroup_only_weighted-unifrac.tsv",
    sep="\t",
    index_col=0,
)
df = pd.read_csv(
    "results/diversity/metaphlan_riskgroup_only_bray-curtis.tsv", sep="\t", index_col=0
)
...
ax = plt.gca()
sns.scatterplot(
    data=pcoa_df, x="PC1", y="PC2", hue="group", palette=palette, s=100, ax=ax
)
...
plt.grid(False)
plt.tight_layout()
plt.savefig("results/sgb_dist_type_riskgroup_plot.png", dpi=300)
plt.close()
